...
from project.postgres_utils import get_products, insert_products_batch, update_similar_sku_column
from project.typesense_utils import insert_data_into_typesense_batch, search_in_typesense
...
async def parse_xml(file_path: str = None, file=None):
    ...
    for event, element in context:
        if event == "start" and element.tag == "category":
        ...
        elif event == "end" and element.tag == "offer":
            offer_data = {
            ...
            offer_data.update(
                {
                    ...
                    "price_after_discounts": (1 - float(offer_data.get("discount", 0)))
                    * float(element.findtext("price", default=0)),
                }
            )
            ...
            if len(offers_batch) >= batch_size:
                await process_offers_batch(offers_batch)
                offers_batch = []
...
async def process_offers_batch(offers_batch):
    inserted_products = await insert_products_batch(offers_batch)
    await insert_data_into_typesense_batch(inserted_products)
